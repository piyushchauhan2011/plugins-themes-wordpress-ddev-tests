# Database scaling (not deployed)

This DDEV site uses **one MariaDB**. WordPress core and Hotel Booking Core talk to it through a single `global $wpdb`. Nothing in this folder is wired into `wp-content/` or `.ddev/`.
