...
PHP 8.2 has **no runtime generics**. PHPStan still type-checks them from PHPDoc: `@template`, `@param T`, `@return list<TOut>`. This project stays at **level 5** in [`phpstan.neon.dist`](../phpstan.neon.dist) (WordPress stubs, no extra PHPStan packages). Run `ddev phpstan`.
